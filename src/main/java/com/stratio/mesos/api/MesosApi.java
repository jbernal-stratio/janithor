...
public class MesosApi {
    ...
    /**
     * Returns the complete address of the api specified by it's parameter
     * @param frameworkName framework name in mesos
     * @param apiPortName name of the api port
     * @return full address for the api port (http://192.168.5.1:8080)
     */
    public Optional<String> findApiEndpoint(String frameworkName, String apiPortName) {
        Call<ResponseBody> mesosCall;
        ...
        try {
            ...
            if (response.code() == HTTPUtils.HTTP_OK_CODE) {
                JsonQuery q = JsonQuery.compile(".frameworks[] | select(.name==\"marathon\") " +
                    "| .tasks[] | select(.name==\""+frameworkName+"\" and .state==\"TASK_RUNNING\") " +
                ...
                if (schedulerIp.isPresent()) {
                    ...
                    Optional<Integer> schedulerPort = lstFrameworks.stream()
                        ...
                        .filter(node -> apiPortName.equals(node.get("name").textValue()))
